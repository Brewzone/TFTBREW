...
//===================================//
//Initialize Timer5 for 400Hz 16bit  //
//===================================//
...
tft.print(F("Brewzone-Systems.co.uk"));
tft.setTextSize(1);
...
//====================================================================================//
// This is where We select to Run with or without a graphical display or to Autotune  //
//====================================================================================//
...
while(true)
{
  ...
  if ((buttons == BUTTON_2 && graph !=1) && (abs(Input - Setpoint[SpPos]) < 0.5))  {StartAutoTune();}

  if (buttons == BUTTON_3 && graph !=1)
  {
  ...
  if (buttons == 4 && graph !=1) {increment = 0.01;}

  if (buttons == 5 && graph !=1) {increment = 1;}

  if (buttons == 6 && graph !=1) {increment = 10;}
            
  if (buttons == BUTTON_UP && graph !=1) {Setpoint[SpPos] += increment; PIDsetpoint = Setpoint[SpPos]; if (Setpoint[SpPos] >=100) {Setpoint[SpPos]=100;}}

  if (buttons == BUTTON_DOWN && graph !=1) {Setpoint[SpPos] -= increment; PIDsetpoint = Setpoint[SpPos]; if (Setpoint[SpPos] <=0){Setpoint[SpPos]=0;}}
  
  if (buttons == BUTTON_BACK && graph !=1) {opState = OFF; pid_menu();}

  if (buttons == BUTTON_FORWARD && graph !=1) {opState = OFF; timer_menu();} // Set boil mash and hop timers  
  ...
  if (ElapsedMin == 0 && SpPos <= 5)
  {
    ...
    if (Setpoint[(SpPos+1)] != 0 && timer[(TmPos+1)] != 0 && stage <= 6)
    {
      ...
      if (stage == 6 || Setpoint[SpPos] == 100 || Setpoint == 0) {opState = OFF;}
...
while(true)
{
  ...
  if (buttons == BUTTON_1) {time_frame = 0; ShowTemp(); time_frame_keys();} // Real Time

  if (buttons == BUTTON_2) {time_frame = 6792; ShowTemp(); time_frame_keys();} // 30 Mins

  if (buttons == BUTTON_3) {time_frame = 13584; ShowTemp(); time_frame_keys();} // 1 Hour

  if (buttons == BUTTON_4) {time_frame = 163018; ShowTemp(); time_frame_keys();} // 12 Hours

  if (buttons == BUTTON_5) {time_frame = 326037; ShowTemp(); time_frame_keys();} // 24 Hours

  if (buttons == BUTTON_6) {time_frame = 2282264; ShowTemp(); time_frame_keys();} // 1 Week

  if (buttons == BUTTON_UP) {time_frame = 4564528; ShowTemp(); time_frame_keys();} // 2 Weeks

  if (buttons == BUTTON_DOWN) {time_frame = 9129056; ShowTemp(); time_frame_keys();}// 1 Month
...
//==============================================//
// Called by ISR every 2.5ms to drive the output //
//==============================================//
...
if (isnan(Kp)) {Kp = 1224.0;} // 1224.0 20.0    The Values 1224.0 2524.5 1198.5 seem to be the best for 18 Liters in an uninsulated HLT made of Stainless Steel

if (isnan(Ki)) {Ki = 2524.5;} // 2524.5 5.0     Rapid heating and quick to settle with a small overshoot of 0.05C even when heating from 10C up to Mash temperature of 66C
 
if (isnan(Kd)) {Kd = 1198.5;} // 1198.5 1.0     Do not be afraid to experiment with high values which seem to go against the accepted values of 20.0 5.0 and 1.0 seen on many
     
if (isnan(TimerMargin)) {TimerMargin = 0.5;} // Brewing forums. 
